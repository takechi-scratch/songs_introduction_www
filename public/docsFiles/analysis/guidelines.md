...
### lyricsVector
Word2Vecを用いて算出された歌詞ベクトルです。（後述）
歌詞ベクトルは著作物には当たらないため、表示・類似度算出に使用して問題ないと考えています。

### lyricsOfficiallyReleased
歌詞ベクトル算出に用いられた歌詞が、公式で掲載されているものかを表します。

ソースの例：
- True ... 動画の説明、MV内の歌詞の正確な文字起こし、公式による歌詞のコメント
- False ... 有志による歌詞のコメント


## 歌詞分析について

歌詞分析・歌詞ベクトルアップロード用のコード（GitHubリポジトリ）は公開準備中です。
今しばらくお待ちください。

### 概要
1. 歌詞をMeCabで単語ごとに分割
2. Word2Vec（モデルはWikipediaから作成）を用いて、単語ごとの数値ベクトルを算出
3. TF-IDFを使った重み付け平均で、単語全体からなる歌詞ベクトルを算出
4. ベクトルのコサイン類似度をMin-Maxで正規化したものを、歌詞の類似度とする
